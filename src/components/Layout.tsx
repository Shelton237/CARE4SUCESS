...
const NAV_MAIN = [
  { to: ROUTE_PATHS.HOME,               label: tt("Accueil") },
  { to: ROUTE_PATHS.SERVICES,            label: tt("Soutien scolaire") },
  { to: ROUTE_PATHS.COURS_DE_LANGUES,    label: tt("Langues") },
  { to: ROUTE_PATHS.COMPETENCES,         label: tt("Compétences") },
  { to: ROUTE_PATHS.COMMENT_CA_MARCHE,   label: tt("Comment ça marche") },
  { to: ROUTE_PATHS.DEVENIR_PROFESSEUR,  label: tt("Devenir coach") },
  { to: ROUTE_PATHS.FAQ,                  label: tt("FAQ") },
];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "Ubuntu, 'Noto Sans', sans-serif" }}>
      ...
      <header className={`sticky top-0 w-full z-50 transition-all duration-300 bg-[#F8F8F7] ${
        ...
        <div className="mx-auto w-full max-w-[1920px] px-6 xl:ps-[6.05%] xl:pe-[4.8%]">
          <div className="flex items-center h-20 xl:h-[69px]">
            ...
            {/* Nav desktop */}
            <nav className="hidden lg:flex items-center ms-2 xl:max-[1399px]:ms-6 min-[1400px]:ms-[6.9%]">
              {NAV_MAIN.map(link => (
                <NavLink
                  key={link.label}
                  to={link.to}
            ...
            {/* CTA desktop */}
            <div className="hidden lg:flex items-center gap-1 xl:gap-[13px] ms-auto ps-1 xl:ps-2">
              <LanguageMenu />
              <NavLink
                to="/login"
              ...
              >
                <User className="w-4 h-4" />{t("Connexion")}
              </NavLink>
              <NavLink
            ...
            <div className="lg:hidden ms-auto flex items-center gap-1">
              <LanguageMenu />
              <button
                onClick={() => setMobileOpen(!mobileOpen)}
                className="p-2 rounded-lg text-[#0D2D5A] hover:bg-white/60 transition-colors cursor-pointer"
                aria-label={t("Menu")}
              >
            ...
            >
              <nav className="container mx-auto px-4 py-4 space-y-1">
                ...
                <div className="pt-3 border-t border-[#0D2D5A]/10 space-y-2">
                  ...
                  <NavLink to={ROUTE_PATHS.PROFESSEURS} onClick={close} className="flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold bg-[#F5A623] text-[#0D2D5A] hover:bg-[#e09520] transition-colors cursor-pointer">{t("Trouver mon coach")}{" "}<ArrowRight className="w-4 h-4" />
                  </NavLink>
                </div>
              </nav>
            </motion.div>
        ...
        <div className="mx-auto w-full max-w-[1920px] px-6 xl:px-[5%] pt-10 xl:pt-[26px]">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr_auto] xl:grid-cols-[368fr_241fr_246fr_244fr_202fr] gap-x-6 gap-y-8 xl:gap-x-0">
            ...
            <div className="xl:ps-[15px]">
              ...
              <p className="text-[13.1px] text-[#A9C1E3] leading-[22px] mt-1.5 xl:mt-[5px] max-w-[266px]">{t("Une plateforme de coaching panafricaine par USRA-CARE.")}
              </p>
            ...
            {/* Parents */}
            <div>
              <h3 className="text-[13.7px] font-extrabold text-[#F5A623] mb-3 xl:mb-[11px] uppercase tracking-[0.1em]">{t("Parents")}</h3>
              <ul className="space-y-2.5 xl:space-y-[11px]">
                {[
                  { label: tt("Évaluation gratuite"), to: ROUTE_PATHS.EVALUATION_GRATUITE },
                  { label: tt("Suivi de progression"), to: ROUTE_PATHS.SERVICES },
                  { label: tt("Comment ça marche"), to: ROUTE_PATHS.COMMENT_CA_MARCHE },
                  { label: tt("FAQ"), to: ROUTE_PATHS.FAQ },
                ].map(item => (
                  <li key={item.label}>
            ...
            {/* Apprenants */}
            <div>
              <h3 className="text-[13.7px] font-extrabold text-[#F5A623] mb-3 xl:mb-[11px] uppercase tracking-[0.1em]">{t("Apprenants")}</h3>
              <ul className="space-y-2.5 xl:space-y-[11px]">
                {[
                  { label: tt("Cours de langues"), to: ROUTE_PATHS.COURS_DE_LANGUES },
                  { label: tt("Compétences pro"), to: ROUTE_PATHS.COMPETENCES },
                  { label: tt("Nos coachs"), to: ROUTE_PATHS.ANNUAIRE_COACHS },
                  { label: tt("FAQ"), to: "/faq?cat=langues" },
                ].map(item => (
                  <li key={item.label}>
                    <NavLink to={item.to} className="block text-[14.5px] text-white/90 leading-5 hover:text-[#F5A623] transition-colors">
                      {t(item.label)}
            ...
            {/* Coachs */}
            <div>
              <h3 className="text-[13.7px] font-extrabold text-[#F5A623] mb-3 xl:mb-[11px] uppercase tracking-[0.1em]">{t("Coachs")}</h3>
              <ul className="space-y-2.5 xl:space-y-[11px]">
                <li>
                  <NavLink to={ROUTE_PATHS.DEVENIR_PROFESSEUR} className="block text-[14.5px] text-white/90 leading-5 hover:text-[#F5A623] transition-colors">{t("Devenir coach")}
                  </NavLink>
                </li>
                <li><a href="#" className="block text-[14.5px] text-white/90 leading-5 hover:text-[#F5A623] transition-colors">{t("Nos critères")}</a></li>
                <li><NavLink to="/faq?cat=devenir-coach" className="block text-[14.5px] text-white/90 leading-5 hover:text-[#F5A623] transition-colors">{t("FAQ coachs")}</NavLink></li>
                <li>
                  <a href="mailto:contact@care4success.com" className="block text-[14.5px] text-white/90 leading-5 hover:text-[#F5A623] transition-colors">contact@care4success.com
                  </a>
                </li>
              </ul>
            </div>
            ...
            {/* Nous suivre */}
            <div className="lg:border-s lg:border-white/12 xl:ps-[41px] lg:ps-8 xl:h-[143px] xl:self-start">
              <h3 className="text-[14.3px] font-semibold text-white mb-3 xl:mb-[16px]">{t("Nous suivre")}</h3>
          ...
          <div className="border-t border-white/12 mt-6 xl:mt-[23px] pt-5 xl:pt-[23px] pb-6 xl:pb-[30px] flex flex-col md:flex-row justify-between items-center gap-3">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 xl:gap-x-[26px] text-[13.4px] text-[#7F9CC7]">
              <a href="#" className="hover:text-[#F5A623] transition-colors">{t("CGU")}</a>
              <NavLink to={ROUTE_PATHS.POLITIQUE_CONFIDENTIALITE} className="hover:text-[#F5A623] transition-colors">{t("Confidentialité")}</NavLink>
              <a href="#" className="hover:text-[#F5A623] transition-colors">{t("Remboursements")}</a>
              <LanguageMenu variant="dark" dropUp />
            </div>
          </div>
        </div>
      </footer>
