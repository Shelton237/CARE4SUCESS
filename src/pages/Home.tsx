...
interface UniverseCard {
  ...
  ctaLabel: string;
  /** null : bouton inactif ("Bientôt disponible") */
  ctaTo: string | null;
  ctaClass: string;
}
...
const UNIVERSES: UniverseCard[] = [
  {
    ...
    title: tt("Soutien scolaire"),
    desc: tt("Nous trouvons le coach adapté pour votre enfant. Évaluation, matching, suivi et facturation transparente."),
    tags: [tt("Primaire"), tt("Collège"), tt("Lycée"), tt("Examens")],
    photo: HOME_IMAGES.soutien,
    ctaLabel: tt("Faire évaluer mon enfant"),
    ctaTo: ROUTE_PATHS.EVALUATION_GRATUITE,
    ctaClass: "bg-[#0F9B8E] text-white hover:bg-[#0c857a]",
  },
  {
    key: "langues",
    icon: Globe,
    ...
    title: tt("Langues"),
    desc: tt("Choisissez votre coach, comparez les prix, réservez et commencez aujourd'hui."),
    tags: [tt("Anglais"), tt("Français"), tt("Espagnol"), tt("+5 langues")],
    photo: HOME_IMAGES.langues,
    ctaLabel: tt("Trouver un coach de langue"),
    ctaTo: ROUTE_PATHS.COACHS_LANGUES,
    ctaClass: "bg-[#F5A623] text-white hover:bg-[#e09520]",
  },
  {
    key: "competences",
    icon: Briefcase,
    ...
    title: tt("Compétences et carrière"),
    desc: tt("Développez une compétence avec un expert capable de vous accompagner vers un objectif concret."),
    tags: [tt("Bureautique"), tt("Data"), tt("Communication"), "+"],
    photo: HOME_IMAGES.competences,
    ctaLabel: tt("Bientôt disponible"),
    ctaTo: null,
    ctaClass: "bg-[#FBE0DD] border border-[#F3C4BF] text-[#D2453A] cursor-default xl:h-[50px] xl:text-[15.5px]",
  },
];
...
function UniverseCardView({ card }: { card: UniverseCard }) {
  const { t } = useT();
  return (
    <div className={`relative overflow-hidden rounded-2xl xl:rounded-[22px] border bg-gradient-to-r rtl:bg-gradient-to-l rtl:bg-gradient-to-r ${card.bg} ${card.border} p-6 md:p-7 xl:pt-[16px] xl:px-[22px] xl:pb-[22px] min-h-[300px] xl:h-[354px] flex flex-col`}>
      {/* Photo à droite, fondue vers la couleur de la carte. Icône décorative si le fichier est absent. */}
      <card.icon className="absolute end-6 top-6 w-28 h-28 opacity-[0.05] xl:hidden" style={{ color: card.color }} aria-hidden />
      {/* Deux masques imbriqués : fondu vers le bas (div) puis vers la gauche (img) */}
      <div
        className="hidden xl:block absolute -end-px -top-px"
        style={{
          width: card.photo.w,
          height: card.photo.h,
      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        <h3 className="text-xl xl:text-[23.6px] font-extrabold text-[#0D2D5A] mb-2.5 xl:mb-[8px] leading-tight">{t(card.title)}</h3>
        <p className="text-[13px] xl:text-[13.7px] leading-relaxed xl:leading-[19px] text-gray-700 xl:text-[#2A3550] mb-4 xl:mb-5 max-w-[92%] xl:max-w-[306px] xl:min-h-[57px]">{t(card.desc)}</p>
        <div className="flex flex-wrap gap-2 xl:gap-[9px] mb-5 xl:mb-3">
          {card.tags.map(tag => (
            <span key={tag} className={`text-xs xl:text-[12.4px] font-semibold px-3 xl:px-[14px] py-1.5 xl:py-[7px] rounded-full ${card.chip}`}>{tag === "+" ? tag : t(tag)}</span>
          ))}
        </div>
        <div className="mt-auto">
          <div className="mb-3.5">
            {card.ctaTo ? (
              <NavLink to={card.ctaTo} className={`${CARD_BTN} ${card.ctaClass}`}>{t(card.ctaLabel)}{" "}<ArrowRight className="w-4 h-4" /></NavLink>
            ) : (
              <span className={`${CARD_BTN} ${card.ctaClass}`}>
                <Clock className="w-4 h-4 xl:w-5 xl:h-5" />{t(card.ctaLabel)}
              </span>
            )}
          </div>
        </div>
...
function ApproachPanel({
  ...
  return (
    <div className={`rounded-2xl xl:rounded-[18px] border p-5 md:p-6 xl:pt-[22px] xl:px-[27px] xl:pb-[21px] xl:h-[271px] bg-gradient-to-br ${teal ? "from-[#DFF5F1] to-[#F3FAF9] border-[#CDEBE6]" : "from-[#FFF1D3] to-[#FFFAEE] border-[#FBE7BA]"}`}>
      <div className={`flex items-center gap-4 xl:gap-[25px] mb-5 xl:mb-[16px] ${teal ? "xl:ps-[11px]" : ""}`}>
        {teal ? (
      ...
      <div className="bg-white/90 rounded-xl xl:rounded-[14px] px-4 py-5 xl:py-0 xl:h-[107px] xl:px-[4px] mb-5 xl:mb-[13px] grid grid-cols-2 gap-y-5 sm:flex sm:items-center sm:justify-between">
        {steps.map((step, i) => (
          <div key={step.label} className="contents sm:flex sm:items-center">
...
const WHY: { icon: LucideIcon | FilledIcon; title: string; desc: string; round: boolean }[] = [
  { icon: BadgeCheck, title: tt("Coachs sélectionnés"), desc: tt("Compétences, expérience et pédagogie vérifiées."), round: true },
  { icon: BarsFilled, title: tt("Accompagnement adapté"), desc: tt("Une solution pour chaque objectif et chaque niveau."), round: false },
  { icon: UsersFilled, title: tt("Progression suivie"), desc: tt("Cours, objectifs et résultats depuis votre espace."), round: false },
  { icon: ShieldFilled, title: tt("Paiements simples et sécurisés"), desc: tt("Mobile Money et carte bancaire selon votre pays."), round: false },
];
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#F7FAFC]" style={{ fontFamily: "Nunito, 'Noto Sans', sans-serif" }}>
        ...
        <div className="mx-auto max-w-[1920px] px-6 xl:ps-[6.05%] xl:pe-0 relative z-10 pt-11 md:pt-12 pb-5 xl:pt-[48px] xl:pb-0">
          <div className="max-w-xl xl:max-w-none">
            <h1
              ...
              style={SERIF}
            >
              {rich(t("Every genius\nneeds <gold>a coach</gold>"), { gold: c => <span className="italic text-gold-shimmer xl:text-[1.145em]">{c}</span> })}
            </h1>
            <div className="text-white/90 text-lg md:text-[22px] xl:text-[23.8px] leading-snug xl:leading-[28px] mt-6 xl:mt-[30px] space-y-2 xl:space-y-[8px]">
              <p>{t("Soutien scolaire. Langues. Compétences.")}</p>
              <p><em className="italic">{t("Un coach pour chaque objectif.")}</em></p>
            </div>
      ...
      <section className="py-10 md:py-12">
        <div className={WRAP}>
          <div className="relative xl:ps-[17px] mb-6 xl:mb-3">
            <p className="text-[#0F9B8E] text-xs xl:text-[12.8px] font-bold uppercase tracking-[0.2em] mb-2 xl:mb-[11px]">{t("Nos univers")}</p>
            <h2 className="text-3xl md:text-[40px] xl:text-[46.7px] font-bold text-[#0D2D5A] leading-tight xl:leading-[1.1]" style={SERIF}>{t("Quel est votre objectif\u00A0?")}
            </h2>
            <p className="text-gray-500 mt-1.5 xl:mt-1">{t("Chaque parcours est unique. Choisissez le vôtre.")}</p>

            <div className="hidden md:block absolute end-0 top-0 xl:-top-[11px] text-end -rotate-[10deg] origin-right rtl:origin-left rtl:origin-right">
              <p className="text-[27px] xl:text-[28.4px] leading-[1.05] text-[#0D2D5A] font-medium" style={HANDWRITING}>
                {rich(t("Un coach\npour chaque objectif"))}
              </p>
              <HandUnderline className="w-20 xl:w-[73px] h-3 ms-auto mt-0.5" />
            </div>
          </div>
      ...
      <section className="py-10 md:py-12">
        <div className={`${WRAP} xl:max-w-[1326px]`}>
          <div className="text-center mb-6 xl:mb-3.5">
            <h2 className="text-2xl md:text-[34px] xl:text-[32.2px] font-bold text-[#0D2D5A] leading-tight xl:leading-[1.2]" style={SERIF}>{t("Deux approches, un même objectif\u00A0: votre réussite")}
            </h2>
            <p className="text-gray-500 mt-2 xl:mt-1 xl:text-[20px] xl:leading-7">{t("Selon votre besoin, nous vous accompagnons différemment.")}</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 xl:gap-[21px]">
            <ApproachPanel
              ...
              steps={[
                ...
                { icon: Users, label: tt("Matching\nCare4Success") },
                { icon: UserCog, label: tt("Coach\nassigné") },
                { icon: TrendingUp, label: tt("Suivi et bilan\nrégulier") },
              ]}
              cta={
                <NavLink to={ROUTE_PATHS.SERVICES} className={`${CARD_BTN} xl:h-[44px] xl:px-[27px] xl:text-[13.2px] bg-[#0F9B8E] text-white hover:bg-[#0c857a]`}>{t("En savoir plus sur le soutien scolaire")}{" "}<ArrowRight className="w-4 h-4" />
                </NavLink>
              }
            />
            <ApproachPanel
              ...
              steps={[
                ...
                { icon: Scale, label: tt("Comparaison\ndes tarifs") },
                { icon: CalendarDays, label: tt("Réservation\ndu cours") },
                { icon: Laptop, label: tt("Cours\nen ligne") },
              ]}
              cta={
                <NavLink to={ROUTE_PATHS.COURS_DE_LANGUES} className={`${CARD_BTN} xl:h-[44px] xl:px-[27px] xl:text-[13.2px] bg-[#F5A623] text-[#0D2D5A] hover:bg-[#e09520]`}>{t("Découvrir les cours de langues")}{" "}<ArrowRight className="w-4 h-4" />
                </NavLink>
              }
            />
          </div>
      ...
      <section className="py-10 md:py-12">
        <div className={WRAP}>
          <h2 className="text-2xl md:text-[32px] font-bold text-[#0D2D5A] text-center mb-7 xl:mb-3" style={SERIF}>{t("Pourquoi choisir Care4Success\u00A0?")}
          </h2>
          <div className="grid sm:grid-cols-2 xl:grid-cols-[334fr_344fr_321fr_335fr] gap-x-8 gap-y-7 xl:gap-x-0 xl:gap-y-0 xl:mt-[22px]">
            {WHY.map((item, i) => (
              <div
                key={item.title}
                className={`flex items-start gap-4 xl:gap-5 xl:h-[68px] ${i === 0 ? "xl:ps-[19px]" : "xl:ps-[10px] xl:border-s xl:border-[#0D2D5A]/10"}`}
              >
                <span
                ...
                <div className="xl:pt-[8px]">
                  ...
                  <p className="text-sm xl:text-[15.4px] text-gray-500 xl:text-[#5C6B80] leading-relaxed xl:leading-[22px] mt-1 xl:mt-[8px] xl:max-w-[232px]">{t(item.desc)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="relative py-10 md:py-11 xl:py-0 xl:h-[229px] bg-[#06274A] overflow-hidden">
        <div className="absolute top-[53px] start-[31px] w-[112px] h-[112px] rounded-full bg-[#0F9B8E]/20 pointer-events-none" />
        <div className="absolute top-[62px] start-0 w-[166px] h-[166px] rounded-full bg-[#8A8467]/30 pointer-events-none" />
        <div className="absolute -top-[60px] -end-[20px] w-[200px] h-[200px] rounded-full bg-[#1A6CC8]/12 pointer-events-none" />
        <div className="absolute top-[86px] end-[48px] w-[120px] h-[120px] rounded-full bg-[#8A8467]/30 pointer-events-none" />
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-10">
            <div className="max-w-xl">
              <h2 className="text-3xl md:text-[38px] xl:text-[34.3px] font-bold text-white leading-tight xl:leading-[1.15]" style={SERIF}>
                {rich(t("Prêt à <gold>atteindre votre objectif</gold>\u00A0?"), { gold: c => <span className="text-[#F5A623]">{c}</span> })}
              </h2>
              <p className="text-blue-100/85 mt-3 xl:mt-[10px] xl:text-[17px] xl:leading-[22.5px] xl:max-w-[540px]">{t("Scolaire, langues ou compétences, Care4Success vous accompagne vers le bon coach.")}
              </p>
            ...
            <div className="hidden md:block -rotate-[12deg] shrink-0 xl:mt-[29px] xl:me-[6px]">
              <p className="text-[28px] leading-[1.1] xl:leading-[1.2] text-white font-medium" style={HANDWRITING}>
                {rich(t("Investir aujourd'hui\ndans votre potentiel"))}
              </p>
              <HandUnderline className="w-[88px] h-3 mt-1 xl:mt-[11px] ms-6 xl:ms-[60px]" />
            </div>
          </div>
        </div>
      </section>
