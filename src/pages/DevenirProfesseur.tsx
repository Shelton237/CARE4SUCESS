...
const HERO_CARD: { icon: typeof Monitor; label: string }[] = [
  ...
  { icon: Clock, label: tt("Horaires flexibles") },
  { icon: Award, label: tt("Revenus selon\nvotre engagement") },
];

const TRUST: { icon: LucideOrFilled; label: string }[] = [
  { icon: ShieldFilled, label: tt("Une plateforme de confiance") },
  { icon: UsersFilled, label: tt("Des apprenants motivés") },
  { icon: BadgeCheck, label: tt("Un accompagnement dédié") },
];
type LucideOrFilled = FilledIcon | typeof BadgeCheck;

const WAYS = [
  {
    key: "soutien", to: ROUTE_PATHS.SERVICES, title: tt("Soutien scolaire"), photo: IMG.soutien,
    desc: tt("Accompagnez les élèves du primaire au lycée dans leurs apprentissages (Maths, français, sciences, etc.)."),
    icon: GraduationCap, iconColor: "#1A6CC8", circle: "bg-[#DDEEFB]", bg: "from-[#EEF8FE] via-[#EBF7FF] to-[#F4FAFF]", border: "border-[#DCEBF7]", fr: "305fr",
  },
  {
    key: "langues", to: ROUTE_PATHS.COURS_DE_LANGUES, title: tt("Langues"), photo: IMG.langues,
    desc: tt("Enseignez une langue et ouvrez de nouvelles opportunités (anglais, français, espagnol, etc.)."),
    icon: Globe, iconColor: "#F5A623", circle: "bg-[#FDEDD0]", bg: "from-[#FFF3E3] via-[#FDF9EE] to-[#FCF8F0]", border: "border-[#F6E9D2]", fr: "290fr",
  },
  {
    key: "competences", to: ROUTE_PATHS.COMPETENCES, title: tt("Compétences pro"), photo: IMG.competences,
    desc: tt("Partagez votre expertise dans des domaines professionnels (data, bureautique, management, communication, etc.)."),
...
const STEPS = [
  { n: "01", title: tt("Candidatez"), desc: tt("Remplissez notre formulaire en quelques minutes."), icon: ClipboardList, fr: 166 },
  { n: "02", title: tt("Nous vérifions votre profil"), desc: tt("Nous étudions vos qualifications et votre expérience."), icon: Search, fr: 177 },
  { n: "03", title: tt("Évaluation / entretien"), desc: tt("Selon votre domaine, un test ou un entretien est réalisé (notamment pour le scolaire)."), icon: UsersRound, fr: 175 },
  { n: "04", title: tt("Profil validé"), desc: tt("Vous rejoignez notre réseau de coachs."), icon: CheckFilled, fr: 164 },
  { n: "05", title: tt("Commencez à coacher"), desc: tt("Vous définissez vos disponibilités et recevez vos premiers apprenants."), icon: CirclePlay, fr: 170 },
];

const WHY = [
  { icon: UsersFilled, title: tt("Partagez votre expertise"), desc: tt("Aidez des apprenants à atteindre leurs objectifs."), round: "bg-white border border-[#CBEBE5] text-[#0F9B8E]" },
  { icon: CalendarDays, title: tt("Gérez votre temps"), desc: tt("Choisissez vos disponibilités en ligne ou en présentiel."), round: "bg-white border border-[#0D2D5A]/8 text-[#0F9B8E]" },
  { icon: BarsFilled, title: tt("Développez votre activité"), desc: tt("Accédez à de nouveaux apprenants et élargissez votre réseau."), round: "bg-[#E7F7F4] border border-[#D6F0EB] text-[#0F9B8E]" },
  { icon: Heart, title: tt("Rejoignez une mission utile"), desc: tt("Contribuez à l'éducation et au développement des talents."), round: "bg-white border border-[#0D2D5A]/8 text-[#E2574C]" },
];

const TESTIMONIALS = [
  {
    quote: tt("Rejoindre Care4Success m'a permis de partager ma passion pour les langues, de rencontrer des apprenants motivés et de vivre une expérience très enrichissante."),
    role: tt("Coach de langues"),
    rating: 5,
  },
];
...
export default function DevenirProfesseur() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      setCompleted(true);
      toast({ title: t("Candidature envoyée"), description: t("Notre équipe vous contacte sous 48h.") });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!verticale) {
      toast({
        title: t("Verticale requise"),
        description: t("Merci de choisir la verticale souhaitée avant d'envoyer votre candidature."),
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#F7FCFF]" style={{ fontFamily: "Nunito, 'Noto Sans', sans-serif" }}>
      ...
      <section className="relative overflow-hidden bg-[#022D58] xl:h-[464px]">
        ...
        <div className="mx-auto max-w-[1920px] px-6 xl:ps-[6.05%] relative z-10 pt-12 pb-10 xl:pt-[48px] xl:pb-0">
          ...
          <h1
            ...
            style={SERIF}
          >{rich(t("Transmettez votre savoir.\nFaites <s1>grandir des potentiels.</s1>"), { s1: c => <span className="italic text-gold-shimmer">{c}</span> })}</h1>
          <p className="mt-6 xl:mt-[25px] text-white/90 text-lg xl:text-[22.8px] leading-snug xl:leading-[31px] max-w-[500px] xl:max-w-[600px]">{t("Rejoignez Care4Success et accompagnez des apprenants vers leurs objectifs.")}
          </p>
          ...
          <div className="flex flex-wrap gap-4 xl:gap-[21px] mt-8 xl:mt-[37px]">
            ...
            >{t("Déposer ma candidature")}{" "}<ArrowRight className="w-4 h-4" />
            </button>
            <NavLink
        ...
        <div className="hidden lg:block absolute end-[5.4%] top-[40px] xl:top-[42px] text-start -rotate-[9deg] origin-left rtl:origin-right z-10">
          <p className="text-[28px] xl:text-[35px] leading-[1.02] xl:leading-[35px] text-white font-medium" style={HANDWRITING}>{rich(t("Votre\nexpertise\nchange\ndes vies"))}</p>
          <HandUnderline className="w-[78px] xl:w-[88px] h-3 mt-1 -ms-1" />
        </div>
      ...
      <section className="py-10 md:py-12">
        <div className={WRAP}>
          <div className="xl:ps-[22px]">
            <p className={EYEBROW}>{t("Trois façons de coacher")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[9px]`} style={SERIF}>{t("Partagez vos compétences dans votre domaine.")}</h2>
            <p className="mt-1.5 xl:mt-[4px] text-[#5C6B80] xl:text-[18.5px]">{t("Choisissez la ou les catégories qui vous correspondent.")}</p>
          </div>
          ...
          <div className="mt-6 xl:mt-[10px] grid grid-cols-1 md:grid-cols-3 gap-5 xl:gap-[22px] xl:[grid-template-columns:305fr_290fr_302fr]">
            {WAYS.map(way => (
              <NavLink
                key={way.key}
                to={way.to}
                aria-label={`${t("Découvrir")} : ${t(way.title)}`}
                className={`group relative block overflow-hidden rounded-2xl xl:rounded-[16px] border bg-gradient-to-r rtl:bg-gradient-to-l rtl:bg-gradient-to-r ${way.bg} ${way.border} p-5 pb-[68px] xl:pt-[8px] xl:ps-[35px] xl:pe-[0px] xl:pb-[22px] min-h-[220px] xl:h-[243px] hover:-translate-y-0.5 transition-transform`}
              >
                <div
                  className="hidden xl:block absolute end-0 top-0"
                  style={{
                ...
                <div className="relative z-10 xl:pe-[var(--pw)]" style={{ ["--pw" as string]: `${way.photo.w - 22}px` }}>
                  ...
                  <h3 className="mt-3 xl:mt-[0px] text-xl xl:text-[23px] font-extrabold text-[#0D2D5A] leading-tight">{t(way.title)}</h3>
                  <p className="mt-2 xl:mt-[8px] text-sm xl:text-[16px] leading-relaxed xl:leading-[21px] text-[#4B5A73]">{t(way.desc)}</p>
                </div>

                <span
                  aria-hidden
                  className="absolute end-4 bottom-4 xl:end-[13px] xl:bottom-[10px] w-10 h-10 xl:w-[43px] xl:h-[43px] rounded-full bg-white text-[#0D2D5A] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform z-10"
                >
      ...
      <section className="py-10 md:py-12">
        <div className={WRAP}>
          <div className="xl:ps-[22px]">
            <p className={EYEBROW}>{t("Comment nous rejoindre ?")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[1px] xl:text-[41.2px]`} style={SERIF}>{t("Un processus clair et équitable.")}</h2>
            <p className="mt-1.5 xl:mt-[4px] text-[#5C6B80] xl:text-[18.5px]">{t("Nous sélectionnons nos coachs pour garantir une expérience de qualité à nos apprenants.")}</p>
          </div>
          ...
          <ol className="mt-6 xl:mt-[12px] min-[1400px]:-mx-[6px] grid gap-4 sm:grid-cols-2 lg:flex lg:items-stretch lg:gap-0">
            {STEPS.map((step, i) => (
              <li key={step.n} className="contents lg:flex lg:items-center lg:min-w-0" style={{ flex: `${step.fr} 1 0%` }}>
                <div className="flex-1 min-w-0 h-full rounded-xl xl:rounded-[14px] border border-[#0D2D5A]/[0.07] bg-white/45 p-4 xl:pt-[20px] xl:px-[14px] xl:pb-[16px] text-center lg:text-start xl:text-center xl:min-h-[191px]">
                  ...
                  <h3 className="mt-3 xl:mt-[16px] font-extrabold text-[#0D2D5A] text-[15px] xl:text-[17.2px] leading-snug">{t(step.title)}</h3>
                  <p className="mt-1.5 xl:mt-[6px] text-[13px] xl:text-[15px] leading-snug xl:leading-[21px] text-[#5C6B80]">{t(step.desc)}</p>
                </div>
                {i < STEPS.length - 1 && (
      ...
      <section className="py-10 md:py-12">
        <div className={WRAP}>
          <div className="xl:ps-[22px]">
            <p className={EYEBROW}>{t("Pourquoi devenir coach ?")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[2px] xl:text-[41.2px]`} style={SERIF}>{t("Plus qu'un travail, un impact.")}</h2>
          </div>
          ...
          <div className="mt-6 xl:mt-[18px] grid sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-6 lg:gap-x-0">
            {WHY.map((item, i) => (
              <div
                key={item.title}
                className={`flex items-start gap-4 xl:gap-[18px] ${i === 0 ? "xl:ps-[15px]" : "lg:ps-5 xl:ps-[20px] lg:border-s lg:border-[#0D2D5A]/10"}`}
              >
                ...
                <div className="xl:pt-[3px]">
                  <p className="font-extrabold text-[#0D2D5A] text-[15px] xl:text-[15.4px] leading-snug">{t(item.title)}</p>
                  <p className="mt-1 xl:mt-[5px] text-sm xl:text-[14.6px] text-[#5C6B80] leading-relaxed xl:leading-[21px] xl:max-w-[196px]">{t(item.desc)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-10 md:py-12">
        <div className={`${WRAP} grid lg:grid-cols-[minmax(0,1fr)_minmax(0,660px)_auto] xl:grid-cols-[471px_638px_1fr] items-center gap-6 xl:gap-0`}>
          <div className="xl:ps-[22px]">
            <p className={EYEBROW}>{t("Ils témoignent")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[10px] xl:text-[35.5px] xl:leading-[43px] max-w-[440px] xl:max-w-[470px]`} style={SERIF}>{rich(t("Des coachs engagés,\ndes parcours inspirants."))}</h2>
          </div>
          ...
          <div className="rounded-xl xl:rounded-[14px] border border-[#0D2D5A]/8 bg-white p-4 xl:py-[20px] xl:px-[22px] flex items-center gap-4 xl:gap-[20px]">
            ...
            <div className="min-w-0">
              <p className="text-[#3E4C66] text-sm xl:text-[16px] leading-relaxed xl:leading-[23px]">« {t(current.quote)} »</p>
              <p className="mt-2 xl:mt-[9px] flex flex-wrap items-center gap-x-3 gap-y-1 xl:gap-[14px]">
                <span className="font-extrabold text-[#0D2D5A] text-sm xl:text-[15px] whitespace-nowrap">{t(current.role)}</span>
          ...
          <div className="flex items-center gap-3 xl:gap-[16px] justify-self-end min-[1400px]:-me-[33px]">
            {[
              { icon: ChevronLeft, label: t("Témoignage précédent"), step: -1 },
              { icon: ChevronRight, label: t("Témoignage suivant"), step: 1 },
            ].map(({ icon: Icon, label, step }) => (
              <button
                key={label}
                type="button"
                aria-label={label}
                onClick={() => setTestimonial(i => (i + step + TESTIMONIALS.length) % TESTIMONIALS.length)}
      ...
      <section className="relative py-10 md:py-11 xl:py-0 xl:h-[211px] bg-[#012650] overflow-hidden">
        <div className="hidden xl:block absolute top-[42px] start-[31px] w-[112px] h-[112px] rounded-full bg-[#0F9B8E]/20 pointer-events-none" />
        <div className="hidden xl:block absolute top-[62px] start-0 w-[166px] h-[166px] rounded-full bg-[#8A8467]/30 pointer-events-none" />
        <div className="absolute -top-[60px] -end-[20px] w-[200px] h-[200px] rounded-full bg-[#1A6CC8]/12 pointer-events-none" />
        <div className="hidden xl:block absolute top-[78px] end-[48px] w-[120px] h-[120px] rounded-full bg-[#8A8467]/30 pointer-events-none" />
        ...
        <div className="mx-auto max-w-[947px] px-6 min-[1400px]:px-0 relative z-10 xl:pt-[26px]">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-10">
            <div className="max-w-xl xl:max-w-[700px]">
              <h2 className="text-3xl md:text-[38px] xl:text-[38.8px] font-bold text-white leading-tight xl:leading-[1.15]" style={SERIF}>{rich(t("Prêt à transmettre ce que <s1>vous savez</s1>\u00A0?"), { s1: c => <span className="italic text-[#F5A623]">{c}</span> })}</h2>
              <p className="text-blue-100/85 mt-3 xl:mt-[8px] xl:text-[19.2px] xl:leading-[26px] xl:max-w-[640px]">{t("Rejoignez une communauté de coachs engagés et faites la différence.")}
              </p>
              <div className="flex flex-wrap gap-4 xl:gap-[21px] mt-7 xl:mt-[24px]">
                ...
                >{t("Une question ? Nous contacter")}
                </NavLink>
              </div>
            </div>

            <div className="hidden md:block -rotate-[12deg] shrink-0 xl:mt-[22px] xl:-me-[62px]">
              <p className="text-[28px] xl:text-[29px] leading-[1.1] xl:leading-[1.2] text-white font-medium" style={HANDWRITING}>{rich(t("Ensemble,\nrévélons les talents\nde demain"))}</p>
              <HandUnderline className="w-[88px] h-3 mt-1 xl:mt-[8px] ms-16" />
            </div>
          </div>
        </div>
      </section>
        ...
        <DialogContent className="max-w-lg max-h-[92vh] overflow-y-auto">
          {completed ? (
            <div className="py-8 text-center">
              ...
              <DialogTitle className="text-xl font-bold text-[#0D2D5A] mb-2">{t("Merci pour votre candidature !")}</DialogTitle>
              <DialogDescription className="text-gray-500">{t("Notre équipe vous contacte sous 48h.")}</DialogDescription>
              <Button className="mt-6 bg-[#0D2D5A] hover:bg-[#0B2545]" onClick={() => setOpen(false)}>{t("Fermer")}</Button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <DialogHeader>
                <DialogTitle className="text-[#0D2D5A] text-lg font-bold">{t("Candidatez maintenant")}</DialogTitle>
                <DialogDescription>{t("Notre équipe vous contacte sous 48h.")}</DialogDescription>
              </DialogHeader>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label htmlFor="prenom">{t("Prénom")}</Label>
                  <Input id="prenom" placeholder={t("Prénom")} value={prenom} onChange={e => setPrenom(e.target.value)} required />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="nom">{t("Nom")}</Label>
                  <Input id="nom" placeholder={t("Nom")} value={nom} onChange={e => setNom(e.target.value)} required />
                </div>
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="email">{t("Email")}</Label>
                <Input id="email" type="email" placeholder={t("votre@email.com")} value={email} onChange={e => setEmail(e.target.value)} required />
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="phone">{t("Téléphone")}</Label>
                <Input id="phone" placeholder={t("+237 6XX XXX XXX")} value={phone} onChange={e => setPhone(e.target.value)} required />
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label>{t("Verticale souhaitée")}</Label>
                  <Select value={verticale} onValueChange={setVerticale}>
                    <SelectTrigger><SelectValue placeholder={t("Choisissez...")} /></SelectTrigger>
                    <SelectContent>
                      {VERTICALES.map(v => <SelectItem key={v.value} value={v.value}>{t(v.label)}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="motivation">{t("Expérience et motivation")}</Label>
                <Textarea
                  id="motivation"
                  rows={4}
                  placeholder={t("Décrivez votre parcours d'enseignement...")}
                  value={motivation}
