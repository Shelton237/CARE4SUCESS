...
const LANGUAGES = [
  { key: "en", label: tt("Anglais"), to: `${DIRECTORY}?langue=Anglais` },
  { key: "fr", label: tt("Français"), to: `${DIRECTORY}?langue=Français` },
  { key: "es", label: tt("Espagnol"), to: DIRECTORY },
  { key: "de", label: tt("Allemand"), to: DIRECTORY },
  { key: "ar", label: tt("Arabe"), to: DIRECTORY },
];

const GOALS: { icon: LucideIcon; title: string; desc: string }[] = [
  { icon: Backpack, title: tt("Études"), desc: tt("Préparez vos études\nen toute confiance.") },
  { icon: Briefcase, title: tt("Carrière"), desc: tt("Boostez vos opportunités\nprofessionnelles.") },
  { icon: Plane, title: tt("Voyages"), desc: tt("Communiquez facilement\npartout dans le monde.") },
  { icon: MessagesSquare, title: tt("Culture"), desc: tt("Découvrez de nouvelles\ncultures et échangez\navec aisance.") },
];

const STEPS: { n: string; icon: LucideIcon; title: string; desc: string }[] = [
  { n: "01", icon: Search, title: tt("Recherchez"), desc: tt("Trouvez des coachs selon la\nlangue, le niveau et l'objectif.") },
  { n: "02", icon: Scale, title: tt("Comparez"), desc: tt("Consultez les profils,\nexpériences et tarifs.") },
  { n: "03", icon: CalendarDays, title: tt("Réservez"), desc: tt("Choisissez le créneau\nqui vous convient.") },
  { n: "04", icon: Laptop, title: tt("Apprenez"), desc: tt("Suivez vos cours en ligne\nou en présentiel.") },
];

const WHY: { icon: LucideIcon | FilledIcon; title: string; desc: string; round: string; color: string }[] = [
  { icon: MedalFilled, title: tt("Coachs vérifiés"), desc: tt("Des profils qualifiés et\névalués par notre équipe."), round: "bg-[#0F9B8E]", color: "text-white" },
  { icon: BarsFilled, title: tt("Apprentissage flexible"), desc: tt("En ligne ou en présentiel,\nselon votre emploi du temps."), round: "bg-white border border-[#0D2D5A]/8", color: "text-[#0F9B8E]" },
  { icon: TargetIcon, title: tt("Progression réelle"), desc: tt("Des objectifs clairs et un\nsuivi régulier."), round: "bg-white border border-[#0D2D5A]/8", color: "text-[#0F9B8E]" },
  { icon: ShieldFilled, title: tt("Paiements sécurisés"), desc: tt("Des transactions simples\net fiables."), round: "bg-white border border-[#0D2D5A]/8", color: "text-[#0F9B8E]" },
];

const TESTIMONIALS = [
  {
    quote: tt("Grâce à Care4Success, j'ai pu améliorer mon anglais et décrocher un nouvel emploi. Mon coach a été à l'écoute et très professionnel."),
    role: tt("Apprenante"),
    rating: 5,
  },
];
  ...
  return (
    <div className="min-h-screen bg-[#F8FCFF]" style={{ fontFamily: "Nunito, 'Noto Sans', sans-serif" }}>
      ...
      <section className="relative overflow-hidden bg-[#002A53] xl:h-[460px]">
        ...
        <div className="mx-auto max-w-[1920px] px-6 xl:ps-[6.05%] relative z-10 pt-10 pb-10 xl:pt-[44px] xl:pb-0">
          ...
          <h1
            ...
            style={SERIF}
          >{rich(t("Parler un\n<s1>nouveau monde.</s1>"), { s1: c => <span className="italic text-gold-shimmer">{c}</span> })}</h1>
          <div className="mt-5 xl:mt-[23px] space-y-2 xl:space-y-[8px] text-white/90 text-lg xl:text-[20px] leading-snug xl:leading-[28px] max-w-[520px] xl:max-w-[720px]">
            <p>{t("Apprenez une langue avec un coach adapté à vos objectifs.")}</p>
            <p>{t("Progressez à votre rythme, en ligne ou en présentiel.")}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 xl:gap-[17px] mt-8 xl:mt-[34px]">
            ...
            >{t("Trouver mon coach de langue")}{" "}<ArrowRight className="w-4 h-4" />
            </NavLink>
            <NavLink
          ...
          <ul className="flex flex-wrap gap-x-8 xl:gap-x-[42px] gap-y-3 mt-8 xl:mt-[40px]">
            {[
              { icon: Globe, label: t("Un large choix de langues") },
              { icon: BarsFilled, label: t("Coachs qualifiés"), alt: true },
              { icon: ShieldFilled, label: t("En ligne ou en présentiel") },
      ...
      <section className="py-10 md:py-12">
        <div className="mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[4.9%] min-[1400px]:pe-[2.6%] grid lg:grid-cols-[minmax(0,1fr)_minmax(0,297px)] gap-6 lg:gap-[22px] items-start">
          <div>
            <p className={EYEBROW_TEAL}>{t("Les langues disponibles")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[5px] text-2xl md:text-[30px] xl:text-[39px]`} style={SERIF}>{t("Quelle langue souhaitez-vous apprendre\u00A0?")}</h2>
            <p className="mt-1.5 xl:mt-[4px] text-[#5C6B80] xl:text-[18.2px]">{t("Choisissez votre langue et trouvez le coach qui vous correspond.")}</p>
          ...
          <div className="rounded-2xl xl:rounded-[14px] border border-[#D5F1EE] bg-[#EBFBFA] p-5 xl:pt-[20px] xl:px-[20px] xl:pb-[22px]">
            <div className="flex items-center gap-4 xl:gap-[14px]">
              ...
              <p className="font-bold text-[#0D2D5A] text-lg xl:text-[19px] leading-snug" style={SERIF}>{rich(t("Une autre langue\nen tête\u00A0?"))}</p>
            </div>
            <p className="mt-3 xl:mt-[13px] text-[#5C6B80] text-sm xl:text-[14.6px] leading-relaxed xl:leading-[21px]">{t("Nous avons des coachs pour bien plus de langues.")}
            </p>
            <NavLink
              to={DIRECTORY}
            ...
            >{t("Voir toutes les langues")}{" "}<ArrowRight className="w-4 h-4" />
            </NavLink>
          </div>
        </div>
      </section>
      ...
      {/* ══════════ VOS OBJECTIFS ══════════ */}
      <section className="py-10 md:py-12">
        <div className="mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[5.4%] min-[1400px]:pe-[5.5%] grid lg:grid-cols-[minmax(0,1fr)_minmax(0,411px)] gap-6 lg:gap-[8px] items-start">
          <div>
            <p className={EYEBROW_TEAL}>{t("Vos objectifs")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[5px] text-2xl md:text-[30px] xl:text-[37.7px]`} style={SERIF}>{t("Apprenez une langue pour aller plus loin")}</h2>
            <p className="mt-1.5 xl:mt-[4px] text-[#5C6B80] xl:text-[18.2px]">{t("Quel que soit votre objectif, nous vous aidons à trouver le bon coach.")}</p>
            ...
            <div className="mt-5 xl:mt-[19px] grid grid-cols-2 sm:grid-cols-4 gap-3 xl:gap-[15px]">
              {GOALS.map(g => (
                <div key={g.title} className="rounded-xl xl:rounded-[10px] bg-white border border-[#0D2D5A]/[0.07] p-4 xl:pt-[18px] xl:px-[16px] xl:pb-[16px] xl:h-[179px]">
                  ...
                  <h3 className="mt-2 xl:mt-[11px] font-extrabold text-[#0D2D5A] text-base xl:text-[17.2px]">{t(g.title)}</h3>
      ...
      {/* ══════════ COMMENT ÇA MARCHE ══════════ */}
      <section className="py-10 md:py-12">
        <div className="mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[3.7%] min-[1400px]:pe-[3.7%]">
          <div className="rounded-2xl xl:rounded-[14px] border border-[#FBE7BA]/70 bg-[#FFF8EA] p-5 md:p-7 xl:pt-[19px] xl:px-[26px] xl:pb-[22px]">
            <p className={EYEBROW_AMBER}>{t("Comment ça marche")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[4px] text-2xl md:text-[30px] xl:text-[33.3px]`} style={SERIF}>{t("Trouvez votre coach en quelques étapes")}</h2>
            <p className="mt-1.5 xl:mt-[4px] text-[#5C6B80] xl:text-[18.2px]">{t("Une expérience simple, flexible et transparente.")}</p>
            ...
            <ol className="mt-5 xl:mt-[19px] grid gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-x-[15px]">
              {STEPS.map((step, i) => (
                <li key={step.n} className="relative rounded-xl xl:rounded-[10px] bg-white/85 border border-[#0D2D5A]/[0.05] p-4 xl:pt-[18px] xl:px-[20px] xl:pb-[12px] xl:h-[149px]">
                  ...
                  <h3 className="mt-2.5 xl:mt-[8px] xl:ps-[78px] font-extrabold text-[#0D2D5A] text-[15px] xl:text-[16.6px]">{t(step.title)}</h3>
                  <p className="mt-1 xl:mt-[3px] xl:ps-[78px] text-[#5C6B80] text-[13px] xl:text-[13.8px] leading-snug xl:leading-[19px]">
                    {lines(t(step.desc))}
                  </p>
                  {i < STEPS.length - 1 && (
                    <ArrowRight className="hidden lg:block absolute top-1/2 -translate-y-1/2 end-[10px] xl:end-[14px] w-4 h-4 xl:w-[18px] xl:h-[18px] text-[#0B2E6B]" strokeWidth={2.2} />
                  )}
                </li>
              ))}
            </ol>
      ...
      <section className="py-10 md:py-12">
        <div className="mx-auto max-w-[1440px] px-6">
          <h2 className="text-center text-xl md:text-[24px] xl:text-[25.5px] font-bold text-[#0D2D5A] uppercase tracking-wide" style={SERIF}>{t("Pourquoi choisir Care4Success\u00A0?")}
          </h2>
        </div>
        <div className="mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[6.2%] min-[1400px]:pe-[4%]">
          <div className="mt-6 xl:mt-[20px] grid sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-6 lg:gap-x-0">
            {WHY.map((item, i) => (
              <div key={item.title} className={`flex items-center gap-4 xl:gap-[16px] ${i === 0 ? "" : "lg:ps-5 xl:ps-[12px] lg:border-s lg:border-[#0D2D5A]/10"}`}>
      ...
      {/* ══════════ TÉMOIGNAGE ══════════ */}
      <section className="py-10 md:py-12">
        <div className="mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[2.7%] min-[1400px]:pe-[2.7%] flex items-center gap-4 xl:gap-0">
          <button
            type="button"
            aria-label={t("Témoignage précédent")}
          ...
          <div className="flex-1 grid lg:grid-cols-[minmax(0,1fr)_minmax(0,727px)] xl:grid-cols-[minmax(0,1fr)_727px] items-center gap-6 lg:ps-8 xl:ps-[24px] xl:pe-[43px]">
            <div>
              <p className={EYEBROW_TEAL}>{t("Ils témoignent")}</p>
              <h2 className={`${H2} mt-2 xl:mt-[8px] text-2xl md:text-[28px] xl:text-[31.5px] xl:leading-[39px]`} style={SERIF}>{rich(t("Une nouvelle langue,{md}de nouvelles opportunités."))}</h2>
            </div>
            ...
            <div className="rounded-xl xl:rounded-[12px] border border-[#0D2D5A]/8 bg-white p-4 xl:py-[20px] xl:px-[20px] flex items-center gap-4 xl:gap-[20px]">
              ...
              <div className="min-w-0">
                <p className="text-[#3E4C66] text-sm xl:text-[16.4px] leading-relaxed xl:leading-[22px]">« {t(current.quote)} »</p>
                <p className="mt-2 xl:mt-[8px] flex flex-wrap items-center gap-x-3 gap-y-1 xl:gap-[14px]">
                  <span className="font-extrabold text-[#0D2D5A] text-sm xl:text-[14.6px] whitespace-nowrap">{t(current.role)}</span>
          ...
          <button
            type="button"
            aria-label={t("Témoignage suivant")}
        ...
        <div className="mx-auto max-w-[947px] px-6 min-[1400px]:px-0 relative z-10 xl:pt-[26px]">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-10">
            <div className="max-w-xl xl:max-w-[700px]">
              <h2 className="text-3xl md:text-[38px] xl:text-[38.5px] font-bold text-white leading-tight xl:leading-[45px]" style={SERIF}>{rich(t("Prêt à parler un <s1>nouveau monde</s1>\u00A0?"), { s1: c => <span className="italic text-[#F5A623]">{c}</span> })}</h2>
              <p className="text-blue-100/85 mt-3 xl:mt-[4px] xl:text-[17.5px] xl:leading-[24px]">{t("Trouvez dès maintenant votre coach de langue.")}
              </p>
            ...
            <div className="hidden md:block -rotate-[9deg] shrink-0 xl:mt-[6px] min-[1400px]:-me-[40px]">
              <p className="text-[28px] xl:text-[32px] leading-[1.1] xl:leading-[1.2] text-white font-medium" style={HANDWRITING}>{rich(t("Apprendre aujourd'hui\npour aller plus loin\ndemain"))}</p>
              <HandUnderline className="w-[96px] h-3 mt-1 xl:mt-[6px] ms-14" />
            </div>
          </div>
        </div>
      </section>
