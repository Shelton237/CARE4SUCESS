...
const WRAP = "mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[5.85%] min-[1400px]:pe-[4.8%]";

type Icon = LucideIcon | FilledIcon;

const STEPS: { n: string; icon: Icon; title: string; desc: string; circle: string; ring?: string }[] = [
  { n: "01", icon: ClipboardList, title: tt("Évaluation du besoin"), desc: tt("Nous comprenons vos objectifs\net votre niveau."), circle: "bg-[#DDF3F0] text-[#0F9B8E]" },
  { n: "02", icon: UsersOutline, title: tt("Matching Care4Success"), desc: tt("Nous vous proposons le coach\nle plus adapté à votre profil."), circle: "bg-[#FFF1CC] text-[#F5A623]" },
  { n: "03", icon: CircleUser, title: tt("Démarrage"), desc: tt("Vous commencez vos cours\nen ligne ou en présentiel."), circle: "bg-[#FCE1E5] text-[#E2596D]", ring: "bg-[#FCE9EC] text-[#E2596D]" },
  { n: "04", icon: BarsOutline, title: tt("Suivi des progrès"), desc: tt("Nous suivons vos avancées\net ajustons si nécessaire."), circle: "bg-[#DDECF6] text-[#0F7F8E]" },
];
...
const PROFILES: { key: string; photo: string; icon: Icon; title: string; desc: string; to: string; bg: string; iconWrap?: string; iconColor: string }[] = [
  {
    key: "parents", photo: IMG.parents, icon: UserRound, title: tt("Parents"), to: ROUTE_PATHS.SERVICES,
    desc: tt("Un accompagnement\npour la réussite scolaire\nde votre enfant."),
    bg: "from-[#F2FEFE] to-[#F6FEFD]", iconWrap: "bg-[#2BB3A3]", iconColor: "text-white",
  },
  {
    key: "apprenants", photo: IMG.apprenants, icon: GraduationCap, title: tt("Apprenants"), to: ROUTE_PATHS.COURS_DE_LANGUES,
    desc: tt("Des cours de langues,\ndes compétences et un\nsuivi personnalisé."),
    bg: "from-[#F1FCFE] to-[#F5FDFE]", iconColor: "text-[#0F9B8E]",
  },
  {
    key: "pros", photo: IMG.pros, icon: Briefcase, title: tt("Professionnels"), to: ROUTE_PATHS.COMPETENCES,
    desc: tt("Développez vos compétences\net avancez vers de nouvelles\nopportunités."),
    bg: "from-[#FFF9F6] to-[#FFFBF8]", iconColor: "text-[#F5A623]",
  },
];

const WHY: { icon: Icon; title: string; desc: string; round: string; color: string }[] = [
  { icon: UsersRound, title: tt("Coachs sélectionnés"), desc: tt("Des profils qualifiés et\nvérifiés."), round: "bg-[#DDF3F0]", color: "text-[#0F9B8E]" },
  { icon: UsersRound, title: tt("Accompagnement\npersonnalisé"), desc: tt("Une solution adaptée\nà vos objectifs."), round: "bg-[#FFF1D5]", color: "text-[#F5A623]" },
  { icon: BarsFilled, title: tt("Progression suivie"), desc: tt("Des résultats concrets\ndans le temps."), round: "bg-[#FCE1E5]", color: "text-[#E25A73]" },
  { icon: ShieldFilled, title: tt("Flexibilité"), desc: tt("Des cours en ligne\nou en présentiel."), round: "bg-[#DDF3F0]", color: "text-[#0F9B8E]" },
];
  ...
  return (
    <div className="min-h-screen bg-[#F8FCFF]" style={{ fontFamily: "Nunito, 'Noto Sans', sans-serif" }}>
      ...
      <section className="relative overflow-hidden bg-[#012853] xl:h-[426px]">
        <div className="hidden md:block absolute top-0 end-0 w-[51.2%]">
          <img
            src={IMG.hero}
            alt=""
        ...
        <div className="mx-auto max-w-[1920px] px-6 xl:ps-[6.35%] relative z-10 pt-10 pb-10 xl:pt-[44px] xl:pb-0">
          ...
          <h1
            ...
            style={SERIF}
          >{rich(t("Un parcours simple\nvers <s1>vos objectifs.</s1>"), { s1: c => <span className="italic text-gold-shimmer">{c}</span> })}</h1>
          <p className="mt-5 xl:mt-[26px] text-white/90 text-lg xl:text-[23px] leading-snug xl:leading-[34px] max-w-[520px] xl:max-w-[760px]">{rich(t("Nous vous accompagnons à chaque étape, de l’évaluation{md}du besoin jusqu’au suivi de vos progrès."))}</p>
      ...
      <section className="py-10 md:py-12">
        <div className={WRAP}>
          <div className="relative">
            <p className={EYEBROW}>{t("4 étapes simples")}</p>
            <h2 className={`${H2} xl:text-[40px] mt-2 xl:mt-[5px]`} style={SERIF}>{t("De votre besoin à vos résultats")}</h2>
            <p className={SUB}>{t("Un accompagnement clair, transparent et entièrement personnalisé.")}</p>

            <div className="hidden lg:block absolute end-[6px] top-[-4px] xl:top-[-2px] text-start -rotate-[8deg] origin-left rtl:origin-right text-[#0D2D5A]">
              <p className="text-[24px] xl:text-[29px] leading-[1.1] xl:leading-[1.2] font-medium" style={HANDWRITING}>{rich(t("Un accompagnement\ndu début à la réussite"))}</p>
              <HandUnderline className="w-[84px] h-3 mt-0.5 ms-16" />
            </div>
          </div>

          <ol className="mt-6 xl:mt-[22px] min-[1400px]:-ms-[15px] grid gap-4 sm:grid-cols-2 lg:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] lg:gap-0 items-stretch">
            {STEPS.map((step, i) => (
              <li key={step.n} className="contents">
                <div className="rounded-xl xl:rounded-[12px] border border-[#0D2D5A]/[0.06] bg-white/80 p-4 xl:pt-[20px] xl:ps-[22px] xl:pe-[16px] xl:pb-[18px] xl:h-[202px]">
                  ...
                  <h3 className="mt-3 xl:mt-[14px] font-extrabold text-[#0D2D5A] text-base xl:text-[19.9px] leading-tight">{t(step.title)}</h3>
      ...
      {/* ══════════ EXPÉRIENCE ADAPTÉE ══════════ */}
      <section className="py-10 md:py-12">
        <div className="mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[3.1%] min-[1400px]:pe-[3%]">
          <div className="rounded-2xl xl:rounded-[16px] border border-[#D9F0F2] bg-[#EFFAFC] p-5 md:p-7 xl:pt-[17px] xl:ps-[24px] xl:pe-[24px] xl:pb-[17px]">
            <p className={EYEBROW}>{t("Une expérience adaptée à vos besoins")}</p>
            <h2 className={`${H2} xl:text-[41.3px] mt-2 xl:mt-[6px]`} style={SERIF}>{t("Selon votre profil, un même objectif\u00A0: votre réussite")}</h2>
            <p className={SUB}>{t("Que vous soyez parent, apprenant ou professionnel, nous vous accompagnons différemment.")}</p>

            <div className="mt-5 xl:mt-[22px] grid md:grid-cols-3 gap-4 xl:gap-[22px]">
              {PROFILES.map(p => (
                <div key={p.key} className={`relative overflow-hidden rounded-xl xl:rounded-[12px] border border-[#0D2D5A]/[0.06] bg-gradient-to-r rtl:bg-gradient-to-l rtl:bg-gradient-to-r ${p.bg} p-5 xl:pt-[22px] xl:ps-[22px] xl:pb-[18px] xl:h-[250px]`}>
                  <img
                    src={p.photo}
                    alt=""
                    onError={(e) => { e.currentTarget.style.display = "none"; }}
                    className="hidden lg:block absolute end-0 top-0 h-full w-[46%] object-cover object-left"
                    style={{
                  ...
                  <div className="relative z-10 flex flex-col h-full lg:max-w-[62%]">
                    ...
                    <h3 className="mt-2 xl:mt-[9px] font-extrabold text-[#0D2D5A] text-lg xl:text-[21px] leading-tight">{t(p.title)}</h3>
                    <p className="mt-1.5 xl:mt-[9px] text-[#5C6B80] text-sm xl:text-[16.8px] leading-snug xl:leading-[24px]">
                      {lines(t(p.desc))}
                    </p>
                    <NavLink to={p.to} className="mt-auto pt-3 inline-flex items-center gap-2 text-[#0E7490] font-bold text-sm xl:text-[16.4px] hover:gap-3 transition-all">{t("En savoir plus")}{" "}<ArrowRight className="w-4 h-4" strokeWidth={2.2} />
                    </NavLink>
                  </div>
                </div>
              ))}
      ...
      <section className="py-10 md:py-12">
        <div className={WRAP}>
          <p className={EYEBROW}>{t("Pourquoi ça fonctionne ?")}</p>
          <h2 className={`${H2} xl:text-[41px] mt-2 xl:mt-[5px]`} style={SERIF}>{t("Une approche humaine et efficace")}</h2>
          <p className={SUB}>{t("Nous combinons technologie et expertise humaine pour vous offrir la meilleure expérience.")}</p>

          <div className="mt-6 xl:mt-[22px] grid sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-6 lg:gap-x-0">
            {WHY.map((item, i) => (
              <div key={item.title} className={`flex items-center gap-4 xl:gap-[16px] ${i === 0 ? "" : "lg:ps-5 xl:ps-[24px] lg:border-s lg:border-[#0D2D5A]/10"}`}>
        ...
        <div className="mx-auto max-w-[947px] px-6 min-[1400px]:px-0 relative z-10 xl:pt-[28px]">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-10">
            <div className="max-w-xl xl:max-w-[740px]">
              <h2 className="text-3xl md:text-[38px] xl:text-[39.5px] font-bold text-white leading-tight xl:leading-[47px]" style={SERIF}>{rich(t("Prêt à commencer <s1>votre parcours</s1>\u00A0?"), { s1: c => <span className="text-[#F5A623]">{c}</span> })}</h2>
              <p className="text-blue-100/85 mt-3 xl:mt-[6px] xl:text-[18px] xl:leading-[25px]">{t("Rejoignez Care4Success et avancez vers vos objectifs.")}
              </p>
            ...
            <div className="hidden md:block -rotate-[9deg] shrink-0 xl:mt-[16px] min-[1400px]:-me-[40px]">
              <p className="text-[28px] xl:text-[32px] leading-[1.1] xl:leading-[1.2] text-white font-medium" style={HANDWRITING}>{rich(t("Un premier pas\nvers de grandes réussites"))}</p>
              <HandUnderline className="w-[96px] h-3 mt-1 xl:mt-[6px] ms-12" />
            </div>
          </div>
        </div>
      </section>
