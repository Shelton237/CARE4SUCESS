...
const WRAP = "mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[5.85%] min-[1400px]:pe-[4.1%]";

const LEVELS: { key: string; label: string; icon: LucideIcon; subjects: string[] }[] = [
  { key: "primaire", label: tt("Primaire"), icon: GraduationCap, subjects: [tt("Mathématiques"), tt("Français"), tt("Anglais"), tt("Sciences"), tt("Physique-Chimie"), tt("SVT")] },
  { key: "college", label: tt("Collège"), icon: BookOpen, subjects: [tt("Mathématiques"), tt("Français"), tt("Anglais"), tt("Physique-Chimie"), tt("SVT"), tt("Histoire-Géo")] },
  { key: "lycee", label: tt("Lycée"), icon: School, subjects: [tt("Mathématiques"), tt("Français"), tt("Anglais"), tt("Physique-Chimie"), tt("SVT"), tt("Philosophie"), tt("Économie"), tt("Informatique")] },
  { key: "examens", label: tt("Examens"), icon: ClipboardCheck, subjects: [tt("Mathématiques"), tt("Français"), tt("Anglais"), tt("Physique-Chimie"), tt("SVT"), tt("Philosophie")] },
];

const TRUST: { icon: FilledIcon | LucideIcon; label: string }[] = [
  { icon: ShieldFilled, label: tt("Coachs sélectionnés") },
  { icon: UsersFilled, label: tt("Accompagnement personnalisé") },
  { icon: BarsFilled, label: tt("Progression suivie") },
];

const STEPS: { n: string; title: string; desc: string; icon: LucideIcon | FilledIcon }[] = [
  { n: "01", title: tt("Évaluation du besoin"), desc: tt("Nous identifions les difficultés,\nobjectifs et attentes."), icon: ClipboardList },
  { n: "02", title: tt("Matching Care4Success"), desc: tt("Nous recherchons le profil\npédagogique le plus adapté."), icon: UsersRound },
  { n: "03", title: tt("Coach assigné"), desc: tt("Votre enfant commence avec\nun coach sélectionné."), icon: UserCog },
  { n: "04", title: tt("Suivi de la progression"), desc: tt("Vous suivez régulièrement\nses progrès."), icon: BarsOutline },
];

const WHY: { icon: LucideIcon | FilledIcon; title: string; desc: string; round: string; color: string; fill?: boolean }[] = [
  { icon: BadgeCheck, title: tt("Coachs sélectionnés"), desc: tt("Compétences, expérience\net pédagogie vérifiées."), round: "bg-[#099F94]", color: "text-white" },
  { icon: UsersFilled, title: tt("Matching personnalisé"), desc: tt("Le coach est choisi selon\nles besoins réels de l'enfant."), round: "bg-white border border-[#0D2D5A]/8", color: "text-[#F5A623]" },
  { icon: BarsFilled, title: tt("Progression suivie"), desc: tt("Les objectifs et progrès\nsont suivis dans le temps."), round: "bg-white border border-[#0D2D5A]/8", color: "text-[#099F94]" },
  { icon: Heart, title: tt("Parents informés"), desc: tt("Vous gardez une vision claire\nde l'accompagnement."), round: "bg-white border border-[#0D2D5A]/8", color: "text-[#E2574C]", fill: true },
];
    ...
    <div className="min-h-screen bg-[#F8FCFF]" style={{ fontFamily: "Nunito, 'Noto Sans', sans-serif" }}>
      ...
      <section className="relative overflow-hidden bg-[#012B54] xl:h-[472px]">
        ...
        <div className="mx-auto max-w-[1920px] px-6 xl:ps-[6.15%] relative z-10 pt-10 pb-10 xl:pt-[44px] xl:pb-0">
          ...
          <h1
            ...
            style={SERIF}
          >{rich(t("Révéler le potentiel\nde <s1>chaque enfant.</s1>"), { s1: c => <span className="italic text-gold-shimmer">{c}</span> })}</h1>
          <div className="mt-5 xl:mt-[22px] space-y-2 xl:space-y-[8px] text-white/90 text-lg xl:text-[20px] leading-snug xl:leading-[28px] max-w-[520px] xl:max-w-[700px]">
            <p>{t("Chaque enfant apprend différemment.")}</p>
            <p>{t("Nous trouvons le coach qui lui correspond et suivons sa progression.")}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 xl:gap-[17px] mt-8 xl:mt-[36px]">
            ...
            >{t("Faire évaluer mon enfant")}{" "}<ArrowRight className="w-4 h-4" />
            </NavLink>
            <button
              type="button"
            ...
            >
              <CirclePlay className="w-5 h-5 xl:w-[26px] xl:h-[26px]" strokeWidth={1.5} />{t("Découvrir notre approche")}
            </button>
          </div>
      ...
      <section className="py-10 md:py-12">
        <div className={`${WRAP} grid lg:grid-cols-[minmax(0,1fr)_minmax(0,422px)] gap-8 lg:gap-[30px] items-start`}>
          <div>
            <p className={EYEBROW}>{t("Un accompagnement adapté")}</p>
            <h2 className={`${H2} mt-2 xl:mt-[6px]`} style={SERIF}>{t("Comment pouvons-nous aider votre enfant\u00A0?")}</h2>
            <p className="mt-1.5 xl:mt-[3px] text-[#5C6B80] xl:text-[19px]">{t("Un accompagnement adapté à son niveau, ses matières et ses objectifs.")}</p>

            <div role="tablist" aria-label={t("Niveau scolaire")} className="mt-6 xl:mt-[22px] grid grid-cols-2 sm:grid-cols-4 gap-3 xl:gap-[14px]">
              {LEVELS.map(l => {
            ...
            <div className="mt-5 xl:mt-[24px] pt-5 xl:pt-[26px] border-t border-[#0D2D5A]/10 flex flex-wrap gap-2.5 xl:gap-[14px]">
              ...
              <span className="px-4 xl:px-[17px] h-9 xl:h-[42px] inline-flex items-center rounded-full bg-[#EBF5FD] border border-[#DCEBF7] text-[#0D2D5A] text-[13px] xl:text-[13.4px]">{t("+ autres matières")}
              </span>
            </div>
          </div>
      ...
      <section id="approche" className="py-10 md:py-12 scroll-mt-24">
        <div className="mx-auto max-w-[1440px] px-6 min-[1400px]:ps-[4.3%] min-[1400px]:pe-[4.1%]">
          <div className="rounded-2xl xl:rounded-[20px] border border-[#D5F1EE] bg-[#EBFBFA] p-5 md:p-7 xl:pt-[19px] xl:px-[30px] xl:pb-[31px]">
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-5">
              <div>
                <p className={EYEBROW}>{t("L'approche Care4Success")}</p>
                <h2 className={`${H2} mt-2 xl:mt-[4px] xl:text-[42px] xl:leading-[42px]`} style={SERIF}>{rich(t("Vous ne choisissez pas{md}un coach au hasard."))}</h2>
                <p className="mt-3 xl:mt-[8px] text-[#3E4C66] xl:text-[18.5px] xl:leading-[25px] max-w-[640px] xl:max-w-[470px]">{t("Nous commençons par comprendre votre enfant, puis nous identifions le coach le plus adapté à ses besoins.")}
                </p>
              </div>
              <NavLink
                to={ROUTE_PATHS.EVALUATION_GRATUITE}
                className="shrink-0 inline-flex items-center gap-2.5 h-12 xl:h-[51px] px-6 xl:px-[27px] rounded-lg xl:rounded-[9px] bg-[#0F9B8E] text-white font-extrabold xl:text-[17px] hover:bg-[#0c857a] transition-colors xl:mt-[36px] xl:me-[4px]"
              >{t("Faire évaluer mon enfant")}{" "}<ArrowRight className="w-4 h-4" />
              </NavLink>
            </div>
              ...
              {STEPS.map((step, i) => (
                <li key={step.n} className="contents lg:flex lg:items-center lg:min-w-0" style={{ flex: "1 1 0%" }}>
                  <div className="flex-1 min-w-0 h-full rounded-xl xl:rounded-[11px] border border-[#0D2D5A]/[0.06] bg-[#F7FBFE] p-4 xl:pt-[17px] xl:px-[16px] xl:pb-[14px] xl:min-h-[160px] xl:bg-white/80">
                    <div className="flex items-center gap-3 xl:gap-[16px] xl:ps-[6px]">
                    ...
                    <h3 className="mt-3 xl:mt-[14px] text-center font-extrabold text-[#0D2D5A] text-[15px] xl:text-[17.8px] leading-snug">{t(step.title)}</h3>
      ...
      <section className="py-10 md:py-12">
        <div className="mx-auto max-w-[1440px] px-6">
          <h2 className="text-center text-2xl md:text-[28px] xl:text-[33px] font-bold text-[#0D2D5A] leading-tight" style={SERIF}>{t("Pourquoi les parents nous confient leur enfant\u00A0?")}
          </h2>
        </div>
        <div className={WRAP}>
          <div className="mt-6 xl:mt-[24px] grid sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-6 lg:gap-x-0">
            {WHY.map((item, i) => (
              <div
                key={item.title}
                className={`flex items-start gap-4 xl:gap-[16px] ${i === 0 ? "" : "lg:ps-5 xl:ps-[14px] lg:border-s lg:border-[#0D2D5A]/10"}`}
              >
      ...
      <section className="py-10 md:py-12">
        <div className={`${WRAP} grid lg:grid-cols-[minmax(250px,0.8fr)_minmax(0,1.2fr)_170px] xl:grid-cols-[430px_minmax(0,658px)_200px] xl:justify-between items-center gap-6 lg:gap-6 xl:gap-[24px]`}>
          <div>
            ...
            <h2 className={`${H2} mt-2 xl:mt-[8px] xl:text-[34px] xl:leading-[42px]`} style={SERIF}>{rich(t("Ce sont <s1>leurs</s1> progrès{xl}qui parlent le mieux."), { s1: c => <em >{c}</em> })}</h2>
          </div>
          ...
          <div className="rounded-xl xl:rounded-[12px] border border-[#0D2D5A]/8 bg-white p-4 xl:py-[22px] xl:px-[22px] flex items-center gap-4 xl:gap-[20px]">
            ...
            <div className="min-w-0">
              <p className="text-[#3E4C66] text-sm xl:text-[15.2px] leading-relaxed xl:leading-[24px]">{t("« Mon fils avait perdu confiance en mathématiques. Après quelques semaines avec son coach, il est devenu beaucoup plus autonome et ses résultats ont progressé. »")}
              </p>
              <p className="mt-2 xl:mt-[9px] flex flex-wrap items-center gap-x-3 gap-y-1 xl:gap-[16px]">
                <span className="font-extrabold text-[#0D2D5A] text-sm xl:text-[14.6px] whitespace-nowrap">{t("Parent d'un élève")}</span>
          ...
          <ul className="flex flex-row lg:flex-col gap-3 xl:gap-[11px]">
            {[
              { icon: TrendingUp, label: t("Progression suivie") },
              { icon: ShieldCheck, label: t("Coach sélectionné selon le besoin") },
            ].map(({ icon: Icon, label }) => (
              <li key={label} className="flex-1 lg:flex-none flex items-center gap-3 xl:gap-[12px] rounded-xl xl:rounded-[14px] bg-[#E9FAF8] px-4 xl:px-[16px] py-2.5 xl:py-[12px]">
                ...
                <span className="text-[#0D2D5A] font-semibold text-[13px] xl:text-[12.6px] leading-tight">{t(label)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="relative py-10 md:py-11 xl:py-0 xl:h-[217px] bg-[#022A52] overflow-hidden">
        <div className="hidden xl:block absolute top-[50px] start-[31px] w-[112px] h-[112px] rounded-full bg-[#0F9B8E]/20 pointer-events-none" />
        <div className="hidden xl:block absolute top-[90px] start-0 w-[166px] h-[166px] rounded-full bg-[#8A8467]/30 pointer-events-none" />
        <div className="hidden xl:block absolute -top-[60px] -end-[20px] w-[200px] h-[200px] rounded-full bg-[#1A6CC8]/12 pointer-events-none" />
        <div className="hidden xl:block absolute top-[84px] end-[48px] w-[120px] h-[120px] rounded-full bg-[#8A8467]/30 pointer-events-none" />
        ...
        <div className="mx-auto max-w-[947px] px-6 min-[1400px]:px-0 relative z-10 xl:pt-[28px]">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-10">
            <div className="max-w-xl xl:max-w-[700px]">
              <h2 className="text-3xl md:text-[38px] xl:text-[35.5px] font-bold text-white leading-tight xl:leading-[38px]" style={SERIF}>{rich(t("Chaque enfant peut progresser{md}avec le <s1>bon accompagnement.</s1>"), { s1: c => <span className="text-[#F5A623]">{c}</span> })}</h2>
              <p className="text-blue-100/85 mt-3 xl:mt-[8px] xl:text-[17.5px] xl:leading-[24px]">{t("Tout commence par comprendre ses besoins.")}
              </p>
            ...
            <div className="hidden md:block -rotate-[10deg] shrink-0 xl:mt-[22px] min-[1400px]:-me-[60px]">
              <p className="text-[28px] xl:text-[32px] leading-[1.1] xl:leading-[1.15] text-white font-medium" style={HANDWRITING}>{rich(t("Chaque potentiel\nmérite d'être accompagné"))}</p>
              <HandUnderline className="w-[130px] h-3 mt-1 xl:mt-[6px] ms-10" />
            </div>
          </div>
        </div>
      </section>
